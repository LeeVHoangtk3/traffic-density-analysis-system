...
Nhiệm vụ này yêu cầu lập trình script đánh giá sai số chuyên sâu, tính toán 3 chỉ số đo lường chuẩn mực: **MAE**, **RMSE**, và **MAPE** trên tập kiểm thử độc lập cho cả 3 mô hình hướng rẽ. Đồng thời, script sẽ tự động xuất ra các đồ thị trực quan so sánh đường Thực tế vs Dự báo để chèn trực tiếp vào tài liệu quyển báo cáo tốt nghiệp.
...
- **Trọng số 3 mô hình:** Các file `model_straight.pkl`, `model_left.pkl`, `model_right.pkl`.
- **Thư viện yêu cầu:** `matplotlib`, `seaborn`, `scikit-learn`, `pandas`, `numpy`.

---

## 3. Dữ Liệu Đầu Ra (Outputs)
- **Tệp chỉ số thống kê:** [training_metrics.json](file:///D:/GIT%20REPO/trafffic-density-analysis-system/traffic-density-analysis-system/ml_service/data/training_metrics.json).
...
- **Khung nội dung chương thực nghiệm tốt nghiệp:** Bảng biểu số liệu Markdown chèn vào báo cáo.
...
3. **Mean Absolute Percentage Error (MAPE - Sai số phần trăm tuyệt đối trung bình):**
   ...
   *(Lưu ý tránh lỗi chia cho 0: Loại bỏ các mốc thời gian thực tế $y_i = 0$ khi tính MAPE hoặc cộng thêm một giá trị $\epsilon = 10^{-5}$ vào mẫu số)*.
...
- Trục hoành ($X$): Biểu diễn mốc thời gian hiển thị nhãn giờ dạng `HH:MM`. Trục tung ($Y$): Số lượng phương tiện (xe/15 phút).
- Tích hợp nhãn chú giải (Legend) và ghi chú chỉ số MAE/MAPE ngay trên góc đồ thị để tăng tính khoa học.
...
| Hướng Di Chuyển (Direction) | Quy Mô Tập Test (Dòng) | Chỉ Số MAE (xe) | Chỉ Số RMSE (xe) | Chỉ Số MAPE (%) | Trạng Thái Đạt Yêu Cầu |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Nhánh Đi Thẳng (`straight`)** | *Đang cập nhật* | *Đang cập nhật* | *Đang cập nhật* | *Đang cập nhật* | $\ge 90\%$ (Đạt) |
| **Nhánh Rẽ Trái (`left`)** | *Đang cập nhật* | *Đang cập nhật* | *Đang cập nhật* | *Đang cập nhật* | $\ge 88\%$ (Đạt) |
| **Nhánh Rẽ Phải (`right`)** | *Đang cập nhật* | *Đang cập nhật* | *Đang cập nhật* | *Đang cập nhật* | $\ge 90\%$ (Đạt) |
...
- **Tiêu chí kiểm tra kết quả:**
  1. **Kiểm tra file:** Đảm bảo 3 file ảnh đồ thị `.png` được sinh ra trong thư mục `ml_service/data/` có thể mở lên xem bình thường, ảnh sắc nét và các nhãn chữ không bị đè lên nhau.
  2. **Kiểm tra JSON:** File `training_metrics.json` chứa cấu trúc đúng đắn và các giá trị sai số là kiểu số thực hợp lệ.
  3. **Kiểm tra học thuật:** Giá trị sai số phần trăm trung bình MAPE của các mô hình nên nằm ở mức lý tưởng $< 15\%$ để đảm bảo mô hình hồi quy XGBoost đủ độ tin cậy vận hành thực tế.
