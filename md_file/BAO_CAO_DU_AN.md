...
- Cung cấp API để xem dữ liệu thô, tổng hợp mức ùn tắc từ database, xem lịch sử tổng hợp và dự báo.
...
- Phần tổng hợp mức ùn tắc ở backend hiện đã có thể đọc trực tiếp từ dữ liệu database, đồng thời vẫn hỗ trợ chế độ tính nhanh từ tham số `vehicle_count`.
- Phần dự đoán mật độ giao thông tiếp theo đã có service riêng, ưu tiên dùng lịch sử `traffic_aggregation`, nếu không đủ dữ liệu sẽ fallback sang lịch sử `vehicle_detections`.
- Cơ sở dữ liệu đã có luồng ghi dữ liệu thật vào `traffic_aggregation` và `traffic_predictions` thông qua endpoint compute, prediction service và script seed.
...
- `GET /raw-data`: trả về dữ liệu phát hiện đã lưu, có hỗ trợ filter và phân trang.
- `GET /aggregation`: có thể nhận `vehicle_count` để tính nhanh hoặc tự tổng hợp từ database.
- `GET /aggregation/history`: xem lịch sử các bản ghi tổng hợp đã lưu.
- `POST /aggregation/compute`: chốt dữ liệu theo cửa sổ thời gian và lưu vào `traffic_aggregation`.
- `GET /predict-next`: tạo dự báo mới từ lịch sử dữ liệu và lưu vào `traffic_predictions`.
- `GET /predictions/history`: xem lại lịch sử dự báo.
- `GET /health`: kiểm tra trạng thái backend và kết nối database.
- `GET /cameras`, `POST /cameras`: quản lý danh sách camera ở mức cơ bản.
...
- API `aggregation` đã có thể tổng hợp từ dữ liệu thật trong database, nhưng vẫn chưa sâu ở mức thống kê theo nhiều cửa sổ thời gian như 1 phút, 5 phút, 1 giờ.
- API `predict-next` đã có prediction service, có thể dùng model trong `ml_service/` hoặc fallback nếu dữ liệu chưa đủ.
- Hai bảng `traffic_aggregation` và `traffic_predictions` đã có luồng ghi dữ liệu thật, nhưng vẫn cần làm giàu thêm metadata và lịch sử theo nhiều kịch bản.
- Đã có API lọc dữ liệu theo thời gian, theo camera và theo loại phương tiện cho `GET /raw-data`.
- Đã có xử lý phân trang cơ bản cho dữ liệu thô, lịch sử aggregation và lịch sử prediction.
- Đã có logging request cơ bản, health check và exception handler chung.
- Chưa có authentication, rate limit, dashboard endpoint và migration chuyên nghiệp.
...
Phần dự báo đã chạy được ở mức service backend. Nếu muốn hoàn thiện hơn, dự án cần thêm:
...
- Hiện `TrafficAggregation` và `TrafficPrediction` đã được dùng thực tế trong pipeline backend, nên báo cáo cần mô tả đúng các endpoint và luồng ghi dữ liệu này.
