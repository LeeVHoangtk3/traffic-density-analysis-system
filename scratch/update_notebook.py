...
def create_notebook():
    # Load database URL from local .env if available
    load_dotenv()
    db_url = os.getenv("DB_URL") or os.getenv("MONGODB_URI") or ""
    
    # Standardize connection string for Colab MongoDB Atlas
    if db_url:
        if "cluster0" in db_url.lower():
            try:
                base_url = db_url.split("?")[0]
                if not base_url.endswith("/"):
                    base_url = base_url.rsplit("/", 1)[0] + "/"
                db_url = f"{base_url}traffic_density?retryWrites=true&w=majority"
            except Exception:
                pass
    if not db_url:
        db_url = "mongodb+srv://nguyenthanhhoa599_db_user:N8ma95RRPPeTFwlf@cluster0.lnzkslq.mongodb.net/traffic_density?retryWrites=true&w=majority"
        ...
        "cells": [
            ...
            },
            {
                ...
                "source": [
                    ...
                    "\n",
                    "# 2. CHẠY VIDEO 3 (CAM01 & CAM02 - HƯỚNG XUÔI)\n",
                    ...
                    "\n",
                    "print(\"🤖 Bắt đầu nhận diện CAM02 (Video 3 - traffic3)...\")\n",
                    "!NO_DISPLAY=true DRY_RUN=false TRAFFIC_CAMERA_ID=cam02 TRAFFIC_VIDEO_SOURCE=data/video/cam02-traffic3.mp4 TRAFFIC_OUTPUT_VIDEO=test_data/output/cam02-traffic3_output.mp4 python -m detection.main\n",
                    "\n",
            ...
            },
            {
                ...
                "metadata": {},
                "source": [
                    "## 📤 Bước 7: Xuất toàn bộ 9 video kết quả phân tích về Google Drive (test_data/output)"
                ]
            },
            {
                "cell_type": "code",
            ...
            },
            {
                ...
                "source": [
                    "%cd /content/traffic-density-analysis-system\n",
                    "# Tạo file mô phỏng lịch sử động co giãn đồng bộ cho các camera xuôi/ngược dòng\n",
                    "script_content = \"\"\"\n",
                    "import os\n",
                    "import cv2\n",
                    ...
                    "    print(f\\\"🔄 Đang co giãn và dịch chuyển thời gian đồng bộ cho '{camera_id}'...\\\")\n",
                    "    det_list = list(db.vehicle_detections.find({\\\"camera_id\\\": camera_id}).sort(\\\"timestamp\\\", 1))\n",
                    "    \n",
                    "    # Gom nhóm theo block video dựa trên khoảng cách thời gian thô giữa các detection (> 60s)\n",
                    "    blocks = []\n",
                    "    current_block = []\n",
                    "    for doc in det_list:\n",
                    ...
                    "    \n",
                    "    # Lấy danh sách độ dài các video thực tế tương ứng\n",
                    ...
                    "    \n",
                    "    # Thiết lập mốc ảo co giãn (Tổng 3 tiếng = 180 phút)\n",
                    "    # Cam 1 & 2 song song (08:00 - 09:53:20 / 113.33 phút = 6800 giây)\n",
                    "    # Cam 3 nối tiếp (09:53:20 - 11:00 / 66.67 phút = 4000 giây)\n",
                    "    total_virtual_seconds = 6800.0 if camera_id in [\\\"cam01\\\", \\\"cam02\\\"] else 4000.0\n",
                    "    camera_start_time = datetime.now(timezone.utc) - timedelta(minutes=180)\n",
                    "    if camera_id == \\\"cam03\\\":\n",
                    "        camera_start_time = camera_start_time + timedelta(minutes=113.33)\n",
                    "        \n",
                    "    current_virtual_start = camera_start_time\n",
                    "    for idx, block in enumerate(blocks):\n",
                    "        block_dur = durations[idx]\n",
                    "        block_virtual_seconds = total_virtual_seconds * (block_dur / total_video_dur)\n",
                    "        block_virtual_end = current_virtual_start + timedelta(seconds=block_virtual_seconds)\n",
                    "        \n",
                    ...
                    "            simulated_time = current_virtual_start + timedelta(seconds=s_idx * step)\n",
                    "            # Lưu dạng naive datetime UTC\n",
                    "            db.vehicle_detections.update_one(\n",
                    "                {\\\"_id\\\": doc[\\\"_id\\\"]},\n",
                    "                {\\\"$set\\\": {\\\"timestamp\\\": simulated_time.replace(tzinfo=None)}}\n",
                    "            )\n",
                    "        current_virtual_start = block_virtual_end\n",
                    "        \n",
                    "    print(f\\\"   [OK] Đã co giãn và phân phối {len(det_list)} bản ghi thành công!\\\")\n",
                    "\n",
                    "print(\\\"✅ Hoàn tất quá trình co giãn thời gian đồng bộ cho toàn bộ camera!\\\")\n",
                    "\"\"\"\n",
                    "\n",
